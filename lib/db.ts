...
export async function getDay(date: string): Promise<{ [hour: number]: string }> {
...
export async function getMonthCounts(
  ...
  const database = await getDb();
  const monthlyAggs = database.collection<MonthlyAggregation>("monthly_aggregations");
...
export async function getAllEntries(): Promise<{ [date: string]: { [hour: number]: string } }> {
  const database = await getDb();
  const entries = database.collection<HourlyEntry>("entries");
...
async function updateWeeklyAggregation(date: string) {
  const database = await getDb();
  const dailyAggs = database.collection<DailyAggregation>("daily_aggregations");
  const weeklyAggs = database.collection<WeeklyAggregation>("weekly_aggregations");
  ...
  const weeklyDailies = await dailyAggs.find({
    date: { $gte: startDate, $lte: endDate }
  }).toArray();
  ...
  const totalEntries = dailyEntries.reduce((sum, daily) => sum + daily.count, 0);
...
async function updateMonthlyAggregation(date: string) {
  ...
  const monthlyDailies = await dailyAggs.find({
...
export type { HourlyEntry, DailyAggregation, WeeklyAggregation, MonthlyAggregation };